...
export default function SelecionarAlimento() {
  ...
  const categoriaId = params.categoriaId || "1";

  const { addItems } = useCalories();

  // exemplo de alimentos — substitua por seu JSON se tiver
    ...
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity style={styles.card} onPress={() => toggleSelecionar(item.id)}>
              <View>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity
          style={[styles.btnAdd, selecionados.length === 0 && styles.btnAddDisabled]}
          onPress={onAdicionar}
...
const styles = StyleSheet.create({
  ...
  bolinha: { width: 26, height: 26, borderRadius: 20, borderWidth: 2, borderColor: "#999" },
  ...
  btnCancel: { flex: 1, marginRight: 8, padding: 12, borderRadius: 10, backgroundColor: "#e6e6e6", alignItems: "center" },
  btnCancelTxt: { fontSize: 16 },

  btnAdd: { flex: 1, marginLeft: 8, padding: 12, borderRadius: 10, backgroundColor: "#2196F3", alignItems: "center" },
